...
(function (exports, Laya) {
    ...
    class Layout extends Laya.Script {
        ...
        applyLayout() {
            ...
            if (this.topUI) {
                this.topUI.top = this.applyStatusBar ? zs.laya.sdk.DeviceService.statusBarHeight() * Laya.stage.height / zs.laya.sdk.DeviceService.screenHeight() : 0;
            ...
            if (this.fullUI) {
                this.fullUI.top = -10;
                this.fullUI.bottom = -10;
                this.fullUI.left = -10;
                this.fullUI.right = -10;
            }
        }
    }
    ...
    class ZhiSeView extends BaseView {
        ...
        onAwake() {
            super.onAwake();
            var layout = this.owner.addComponent(Layout);
            if (zs.laya.game.AppMain.appConfig.platformType && zs.laya.game.AppMain.appConfig.platformType != "wx") {
                layout.initLayout(true);
            } else {
                layout.initLayout(false);
            }
        }
        ...
        onEnable() {
            ...
            if (zs.laya.game.AppMain.appConfig.useWebAdApi) {
                var platformCfg = zs.laya.platform.PlatformMgr.platformCfg;//window["platformCfg"]
                if (platformCfg) {
                    var bannerCfg = platformCfg.bannerCfg; //
                    if (bannerCfg) {
                        if (!this.owner.getComponent(zs.laya.platform.BannerCtrl)) {
                            this.owner.addComponent(zs.laya.platform.BannerCtrl);
                        }
                    }

                    var exportGameCfg = platformCfg.exportGameCfg;
                    if (exportGameCfg && exportGameCfg[this.viewName]) {
                        if (!this.owner.getComponent(zs.laya.platform.ExportGameCtrl)) {
                            this.owner.addComponent(zs.laya.platform.ExportGameCtrl);
                        }
                    }
                    var nativeAdCfg = platformCfg.nativeAdCfg;
                    if (nativeAdCfg && nativeAdCfg[this.viewName] && nativeAdCfg[this.viewName].auto != false) {
                        if (!this.owner.getComponent(zs.laya.platform.NativeAdsCtrl)) {
                            this.owner.addComponent(zs.laya.platform.NativeAdsCtrl);
                        }
                    }
                    var customCfg = platformCfg.customCfg;
                    if (customCfg && customCfg[this.viewName]) {
                        if (!this.owner.getComponent(zs.laya.platform.CustomAdCtrl)) {
                            this.owner.addComponent(zs.laya.platform.CustomAdCtrl);
                        }
                    }
                    var mistakenlyTouchCfg = platformCfg.mistakenlyTouchCfg;
                    if (mistakenlyTouchCfg && mistakenlyTouchCfg[this.viewName]) {
                        if (!this.owner.getComponent(zs.laya.platform.MistakenlyTouchCtrl)) {
